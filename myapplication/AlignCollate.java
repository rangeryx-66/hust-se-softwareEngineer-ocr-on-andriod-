...
public class AlignCollate {
    ...
    public static void main(String[] args) {
        ...
        AlignCollate alignCollate = new AlignCollate(32, 100, false, 1.5);
        List<Mat> images = new ArrayList<>();
        images.add(Imgcodecs.imread("test_image.png", Imgcodecs.IMREAD_GRAYSCALE));

        Mat[] processedImages = alignCollate.call(images);
        for (int i = 0; i < processedImages.length; i++) {
            Imgcodecs.imwrite("processed_image_" + i + ".png", processedImages[i]);
        }
    }
}
